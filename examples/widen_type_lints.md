# Eugene 🔒 lint report of `examples/widen_type.sql`
...
Changed type of column `price` to `pg_catalog.int8` in `.books`. This operation requires a full table rewrite with `AccessExclusiveLock` if `pg_catalog.int8` is not binary compatible with the previous type of `price`. Prefer adding a new column with the new type, then dropping/renaming.
